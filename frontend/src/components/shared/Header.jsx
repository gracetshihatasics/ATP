  ...
  return (
    <div style={{ background:"#0a0e12", borderBottom:"0.5px solid #1e3a5f", padding:"9px 18px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
        ...
        <span style={{ fontSize:9, color:"#2d6aad", letterSpacing:"0.08em" }}>v0.5</span>
      </div>
      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
        ...
        <button className={`nb ${mainView==="git"?"on":""}`}       onClick={() => setMainView("git")}>🔗 Git</button>
        <button className={`nb ${mainView==="results"?"on":""}`}   onClick={() => setMainView("results")} style={{ position:"relative" }}>
          📊 Results
          {resultsBadge > 0 && (
            <span style={{ position:"absolute", top:-4, right:-4, background:"#ff3b3b", color:"#fff", fontSize:8, fontWeight:700, width:14, height:14, borderRadius:"50%", display:"flex", alignItems:"center", justifyContent:"center", border:"1px solid #080c0f" }}>
