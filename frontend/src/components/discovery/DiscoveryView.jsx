        ...
        {!plan ? (
          ...
          <>
            <div style={{ background:"#0a0e12", borderBottom:"0.5px solid #1e3a5f", padding:"0 14px", display:"flex", alignItems:"center", overflowX:"auto" }}>
              {[["usecases",`USE CASES (${plan.useCases.length})`],["endpoints",`API (${plan.apiEndpoints?.length??0})`],["suites",`SUITES (${plan.suggestedSuites?.length??0})`]].map(([t,l])=>(
            ...
            {activeTab === "usecases" && (
              <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
                <UseCaseList
                  useCases={filteredUCs} selectedId={selectedUC?.id} savedSuite={savedSuite}
                  filterPriority={filterPriority} setFilterPriority={setFilterPriority}
                  filterCategory={filterCategory} setFilterCategory={setFilterCategory}
                  categories={categories} onSelect={generateScenario}
                  onToggleSuite={toggleSuite} onRun={uc => onLaunchRun(uc)}
                />
            ...
            {activeTab === "endpoints" && <EndpointList endpoints={plan.apiEndpoints} />}
            {activeTab === "suites" && (
              <SuiteList suites={plan.suggestedSuites} useCases={plan.useCases} savedSuite={savedSuite}
                onRunSuite={s => onLaunchRun(null, plan.useCases.filter(u => s.useCaseIds.includes(u.id)))}
